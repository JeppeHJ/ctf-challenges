This challenge is based on an actual bug I found and reported on a major Danish website. It showcases how session-management via localStorage can be dangerous if 1) user-input is not sanitized and validated and 2) if there is no Content-Security Policy (CSP) headers set. This allows an attacker to leak a victim's access-token and hijack their session.
